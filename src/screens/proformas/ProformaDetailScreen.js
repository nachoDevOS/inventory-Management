...
export default function ProformaDetailScreen({ route, navigation }) {
    ...
    Alert.alert('Cambiar Estado', `¿Cambiar a "${nuevoEstado}"?`, [
      { text: 'Cancelar', style: 'cancel' },
      {
        text: 'Confirmar', onPress: async () => {
          await db.runAsync('UPDATE proformas SET estado = ? WHERE idproforma = ?', [nuevoEstado, idproforma]);
          cargar();
        }
      }
    ]);
  ...
  const convertirAVenta = async () => {
    if (proforma?.estado === 'convertida') {
      Alert.alert('Info', 'Esta proforma ya fue convertida a venta');
      return;
    }
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ padding: 16 }}>
      ...
      <View style={[styles.card, STYLES.shadow]}>
        ...
        <InfoRow label="Fecha" value={proforma.fecha} />
        {proforma.detalle ? <InfoRow label="Detalle" value={proforma.detalle} /> : null}
      </View>
      ...
      {/* Botón PDF */}
      <TouchableOpacity
        style={[styles.btnPDF, generandoPDF && { opacity: 0.7 }]}
        onPress={exportarPDF}
        disabled={generandoPDF}
      >
        {generandoPDF
          ? <ActivityIndicator color={COLORS.white} />
          : <Text style={styles.btnPDFText}>📄 Exportar PDF / Compartir</Text>
        }
      </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  totalLabelSub: { fontSize: 13, color: COLORS.textLight },
  ...
  btnConvertir: {
    backgroundColor: COLORS.success, borderRadius: 10, padding: 14, alignItems: 'center', marginBottom: 16,
  },
  btnConvertirText: { color: COLORS.white, fontWeight: 'bold', fontSize: 16 },
  convertidaBadge: {
    backgroundColor: '#DCFCE7', borderRadius: 8, padding: 12, alignItems: 'center', marginBottom: 16,
  },
  convertidaText: { color: COLORS.success, fontWeight: '600', fontSize: 14 },
  btnPDF: {
    backgroundColor: '#7C3AED', borderRadius: 10, padding: 13,
    alignItems: 'center', marginBottom: 12,
  },
  btnPDFText: { color: COLORS.white, fontWeight: 'bold', fontSize: 15 },
});
